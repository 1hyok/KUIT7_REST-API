...
public class MenuController {
    ...
    public ApiResponse<Long> create(@PathVariable Long restaurantId,   // @PathVariable: URL '경로'의 {restaurantId} 자리 값을 꺼내 이 파라미터로 받음 (POST /restaurants/3/menus → 3)
                                    @AuthenticationPrincipal Jwt jwt,  // 검증된 토큰 — 이 가게의 주인인지 확인하는 데 사용
        ...
        return ApiResponse.of(menuService.create(restaurantId, req, AuthPrincipal.memberId(jwt), AuthPrincipal.isAdmin(jwt)));
    }
