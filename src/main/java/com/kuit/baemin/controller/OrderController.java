...
public class OrderController {
    ...
    // @AuthenticationPrincipal Jwt: 검증된 access 토큰. sub에 주문자(로그인 회원) id가 들어있음
    // @RequestBody: 요청 본문(JSON)을 OrderCreateRequest 객체로 변환 / @Valid: 그 객체의 검증 제약 검사
    public ApiResponse<Long> create(@AuthenticationPrincipal Jwt jwt,
                                    @Valid @RequestBody OrderCreateRequest req) {
        Long memberId = Long.valueOf(jwt.getSubject());                            // 주문자는 토큰에서만 — 본문으로 받지 않아 사칭 불가
        return ApiResponse.of(orderService.create(memberId, req));                 // 생성된 주문의 PK(Long)를 공통 응답 포맷에 담아 반환
    }
    ...
    public ApiResponse<PageResponse<OrderResponse>> listByMember(
            ...
            @AuthenticationPrincipal Jwt jwt,   // 검증된 access 토큰. sub에 로그인한 회원 id가 들어있음
    ...
    public ApiResponse<OrderStatus> changeStatus(@PathVariable Long orderId,
                                                 @AuthenticationPrincipal Jwt jwt,   // 검증된 토큰 — 그 주문 가게의 주인인지 확인용
                                                 @Valid @RequestBody OrderStatusUpdateRequest req) {
        // 상태 전이 적법성 + 가게 주인 검증은 서비스가 담당. 여기선 요청 상태값과 호출자 정보를 넘김
        return ApiResponse.of(orderService.changeStatus(orderId, req.getStatus(), AuthPrincipal.memberId(jwt), AuthPrincipal.isAdmin(jwt)));
    }
    ...
    public ApiResponse<Void> cancel(@PathVariable Long orderId,           // 취소할 주문 PK는 URL 경로에서
                                    @AuthenticationPrincipal Jwt jwt) {   // 요청자 식별용 memberId는 토큰(sub)에서
        Long memberId = Long.valueOf(jwt.getSubject());
        orderService.cancel(orderId, memberId);   // 주문이 이 회원 것인지(본인 주문)는 OrderService가 검증(ORDER_FORBIDDEN)
