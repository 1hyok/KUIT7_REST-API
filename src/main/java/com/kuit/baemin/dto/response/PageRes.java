...
public class PageRes<T> {
     ...
     * 이 타입(PageRes)으로 변환해 만든다"는 뜻의 정적 팩토리 관례 이름일 뿐이다
     ...
     */
    public static <T> PageRes<T> from(Page<T> page) {   // page: JPA가 페이징 조회로 돌려준 결과(이번 페이지 데이터 + 페이지 메타정보)
        return PageRes.<T>builder()                     // 빌더 시작. <T>는 "이 PageRes가 담을 데이터 타입"을 명시 (정적 메서드라 추론이 안 돼 직접 알려줌)
                ...
                .build();                               // 위에서 채운 값들로 PageRes 객체를 완성해 반환
    }
}
